...
- **Button hierarchy**: owner-drawn (`BS_OWNERDRAW` + `WM_DRAWITEM` → `DrawFlowButton`), resolved by control ID. Each section has one **hero** (Record/Play/Start) — full-width, tall (`HERO_H`), filled with the action's saturated color, with a right-aligned **hotkey hint** (drawn from the live `g_app.hotkey*` via `GetKeyName`). Footer buttons are **ghost** (white fill + border). **Stop All is a neutral ghost until something is actually running**, then it turns filled-red. `Darken()` makes the pressed shade.
...
- **Inline numeric fields**: `EDIT_SPEED` / `EDIT_LOOPS` / `EDIT_INTERVAL` use the monospace `g_fonts.mono`, committed on `EN_KILLFOCUS` via `Apply*Edit` (clamped + reformatted). `WM_CTLCOLOREDIT` keeps them white. State changes call `UpdateStatusDisplay()` → `RedrawWindow(... RDW_INVALIDATE | RDW_ALLCHILDREN)` so the painted surface and the owner-draw buttons/toggles all refresh.
