...
- **Inline numeric fields**: `EDIT_SPEED` / `EDIT_LOOPS` / `EDIT_INTERVAL` are **borderless** monospace (`g_fonts.mono`) edits over a soft `TRACK_BG` rounded pill drawn by `PaintUI`; the pill gets an accent focus ring when that edit has focus (repainted on `EN_SETFOCUS`/`EN_KILLFOCUS`). Values commit on `EN_KILLFOCUS` via `Apply*Edit` (clamped + reformatted). State changes call `UpdateStatusDisplay()` → `RedrawWindow(... RDW_INVALIDATE | RDW_ALLCHILDREN)`. **MinGW `swprintf` note:** use `%ls` for `wchar_t*` args (plain `%s` is treated as narrow and silently truncates).
